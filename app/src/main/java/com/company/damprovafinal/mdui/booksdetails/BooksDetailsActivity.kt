...
import com.company.damprovafinal.databinding.ActivityEntityitemBinding
...
import com.company.damprovafinal.mdui.EntitySetListActivity
 ...
 * - [BooksDetailsCreateFragment]: to create or edit a(n) BooksDetails,
 * - [BooksDetailsDetailFragment]: to display the details of a(n) BooksDetails,
 * - [BooksDetailsListFragment]: to list BooksDetails.
 *
 ...
 * what to do.
 */
class BooksDetailsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<BooksDetails> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, BooksDetailsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: BooksDetails?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link BooksDetailsCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: BooksDetails?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, BooksDetailsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, BooksDetailsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = BooksDetailsCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: BooksDetails?) {
        val updateFragment = BooksDetailsCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as BooksDetailsActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[BooksDetailsViewModel::class.java]
