...
import com.company.damprovafinal.app.WelcomeActivity
...
import com.company.damprovafinal.app.SAPWizardApplication
...
import com.company.damprovafinal.app.CrashReportConsentFlow
...
import com.company.damprovafinal.app.UsageConsentFlow
...
class SettingsFragment : PreferenceFragmentCompat() {
    ...
    private fun prepareUsagePreference() {
        setUsagePermission!!.isChecked = UsageBroker.isStarted()
        setUsagePermission!!.setOnPreferenceClickListener {
            if (!(it as SwitchPreference).isChecked) {
                SDKInitializer.getService(UsageService::class)!!.stopUsageBroker(true)
                usageUploadPreference.isEnabled = false
                UserSecureStoreDelegate.getInstance().updateConsentStatus(ConsentType.USAGE, false)
            } else {
                val flowContext = flowContext.copy(
                    flow = UsageConsentFlow(requireActivity().application)
                )
                start(requireActivity(), flowContext) { requestCode, _, _ ->
                    if (requestCode == FlowConstants.FLOW_ACTIVITY_REQUEST_CODE) {
                        setUsagePermission!!.isChecked = UsageBroker.isStarted()
                        usageUploadPreference.isEnabled = UsageBroker.isStarted()
                    }
                }
            }
            false
        ...
        usageUploadPreference.setOnPreferenceClickListener {
            ...
            SDKInitializer.getService(UsageService::class)?.also { usageService ->
                usageService.uploadUsageData(forceUpload = false, owner = this, listener = object :
                    ServiceListener<Boolean> {
                    override fun onServiceDone(result: ServiceResult<Boolean>) {
                        usageUploadPreference.isEnabled = true
                        when (result) {
    ...
    private fun prepareCrashReportPreference() {
        ...
        setCrashPermission!!.isChecked = SDKInitializer.getService(CrashService::class)!!.consented
        setCrashPermission!!.setOnPreferenceClickListener{
            if (!(it as SwitchPreference).isChecked) {
                SDKInitializer.getService(CrashService::class)!!.consented = false
                UserSecureStoreDelegate.getInstance().updateConsentStatus(ConsentType.CRASH_REPORT, false)
            } else {
                val flowContext = flowContext.copy(
                        flow = CrashReportConsentFlow(requireActivity().application)
                )
                start(requireActivity(), flowContext) { requestCode, _, _ ->
                    if (requestCode == FlowConstants.FLOW_ACTIVITY_REQUEST_CODE) {
                        setCrashPermission!!.isChecked = SDKInitializer.getService(CrashService::class)!!.consented
